...
class Runner extends ProviderAware {
  ...
  constructor(options = {}) {
    ...
    if (this.options.dumpStateFilename) {
      if (!path.isAbsolute(this.options.dumpStateFilename) &&
        this.options.workingDirectory &&
        this.options.dumpState) {
        this.options.dumpStateFilename = path.join(this.options.workingDirectory, this.options.dumpStateFilename);
      }
    } else {
      this.options.dumpStateFilename = this.commandResultTransport.buildFilename('json', 'state');
    }
  }
  ...
  getCommandMessage(command, decoration = true) {
    ...
    const summary = this.getCommandSummary(command);
  ...
  async read(globPathsAndScripts, state) {
    ...
    const scriptFiles = scripts.map((script, i) => `${i+1}. ${script.filename}`);
    this.spinner.info(`Will run ${scripts.length} playbooks:\n\t${scriptFiles.join('\n\t')}`);
    const results = [];
    await Promise.each(scripts, async (commands) => {
      try {
        ...
        results.push(result);
        this.spinner.succeed(`Ran: ${commands.filename} OK.`);
        this.commandLogger.debug(`--- End ${commands.filename} ---`);
      } catch (err) {
        this.spinner.fail(`FAILED: ${commands.filename}\n\t\t${err} ---\n${err.stack}`);
        this.commandLogger.debug(`--- End ${commands.filename} ---`);
      }
    }, { concurrency: 1 });
    if (this.options.dumpState) {
      this.spinner.start(`Writing state to file '${this.options.dumpStateFilename}...'`);
      let output;
      if (Array.isArray(this.options.dumpStatePath)) {
        output = {};
        this.options.dumpStatePath.forEach(path => {
      ...
      fs.writeFileSync(this.options.dumpStateFilename, serialized, 'utf8')
      this.spinner.succeed(`Wrote state to file '${this.options.dumpStateFilename}' OK.`);
    }
    return results;
  }
  ...
  async run(command, state) {
    ...
    if (Array.isArray(command)) {
      ...
      this.initOraCompatibleLogger(`${state.commandIndex} of ${state.commandCount}`);

      this.spinner.start(`📡 Running ${state.commandCount} commands...`);
  ...
  initOraCompatibleLogger(message) {
    ...
    if (this.options.spinner) {
      this.spinner = this.spinner || ora(message);
      return this.spinner;
    }
    ...
    if (this.options.logger) {
      ...
      this.spinnerspinner.info = bindMethod(this.options.logger.debug, function(message) {
