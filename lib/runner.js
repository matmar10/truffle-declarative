...
class Runner extends ProviderAware {
  ...
  async read(globPathsAndScripts, state) {
    ...
    if (this.options.dump) {
      this.spinner.start(`Writing state to file '${this.options.dumpFilename}...'`);
      let output;
      ...
      this.spinner.succeed(`Wrote state to file '${this.options.dumpFilename}' OK.`);
    }
    this.spinner.succeed(`🏁 Finished OK.`);
    return results;
  }
