...
class Runner extends ProviderAware {
  ...
  contractAt(contractName, at) {
    const Contract = this.getContract(command.contract);
  ...
  async run(command, state) {
    this.spinner = this.spinner || ora(`${commandIndex} of ${commandCount}`);
    ...
    try {
      ...
      if (command.isStatic) {
        ...
        target = this.contractAt(command.contract, at);
      }
