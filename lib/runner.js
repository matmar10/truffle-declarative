  ...
  constructor(options = {}) {
    ...
    if (this.options.mapping) {
      if ('string' === typeof this.options.mapping || Array.isArray(this.options.mapping)) {
        this.mapperOptions = this.scriptReader.merge(this.options.mapping);
      } else {
        this.mapperOptions = this.options.mapping;
      }
    }

    this.mapper = new TruffleObjectMapper(merge(options, this.mapperOptions));
  }
  ...
  async run(command, state) {
    ...
    try {
      ...
      if (this.options.debug) {
        this.spinner.info(`Inputs: ${util.inspect(sortedInputs, { depth: this.options.debugInspectDepth })}`);
      }
      ...
      if (0 === command.run.indexOf('util.')) {
        ...
        if(artifact) {
          ...
          if(type) {
            const transformObject = await this.mapper.buildOutputMapping(method, type, this.mapperOptions.mapping || {});
            transformObject.forEach( transformFn => {
              output = transformFn.transform(output);
            });
            // NOTE: previous like is effectively the same as:
            // const customMapping = this._mapping[key] ?
            //  this._mapping[key] :
            //  mapping[key];
            // need this since one line of merging mapping rules
            // is defined inside truffle-object-mapper in map method
            // ideally should move to encapsulate all inside buildOutputMapping
            // transform value using given callback, if any
          }
      ...
      if (this.options.debug) {
        this.spinner.info(`Outputs: ${util.inspect(output, { depth: this.options.debugInspectDepth })}`);
      }
