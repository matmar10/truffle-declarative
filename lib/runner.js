...
class Runner extends ProviderAware {
  ...
  constructor(options = {}) {
    ...
    this.mapper = new TruffleObjectMapper(options);
    if (this.options.mapping) {
      this.mapping = this.scriptReader.merge(this.options.mapping);
    }
  }
  ...
  async map(contractName, at, map) {
    map = map || this.mapping;
    return await this.mapper.map(contractName, at, map);
  }
