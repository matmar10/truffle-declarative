...
class Runner extends ProviderAware {
    ...
    if (this.options.mapping) {
      if ('string' === typeof this.options.mapping || Array.isArray(this.options.mapping)) {
        ...
        this.mapperOptions = merge(this.options, {
          mapping: loadedMappingFromFiles
        });
      } else {
        // mapping specified as object
        this.mapperOptions = merge(this.options, this.options.mapping);
      }
    } else {
      this.mapperOptions = {};
    }
  ...
  async read(globPathsAndScripts, state = {}) {
    ...
    // TODO: this may be dangerous, but we assume people want to provide the inputs to state
  ...
  async run(command, state) {
    ...
    try {
      ...
      if (0 === command.run.indexOf('util.')) {
        ...
        const outputMapping = this.mapper.buildOutputMapping(command.run, type, this.mapperOptions.mapping || {});
        if ('object' === typeof outputMapping) {
          if (Array.isArray(outputMapping)) {
            output = outputMapping[0].transform(output);
          } else {
            output = outputMapping.transform(output);
          }
        }
