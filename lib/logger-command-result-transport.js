...
class CommandResultTransport extends Transport {
  constructor(opts) {
    ...
    this.filename = this.buildFilename('yml', 'results');
    this.touch(date);
  }

  buildFilename(extention, extra = '') {
    return path.join(this.resultDir, `${this.startTime}-${extra}.${extention}`);
  }
