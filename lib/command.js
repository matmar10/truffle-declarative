...
class Command {
  constructor(definition) {
    ...
    assertValidateOptions(Command.schema, definition, 'Command');

    if (-1 === Command.staticMethods.indexOf(definition.run)) {
      assertValidateOptions(Command.schemaInstanceMethod, definition, 'Command(instance)');
      this.isStatic = false;
    } else {
      assertValidateOptions(Command.schemaStaticMethod, definition, 'Command(static)');
      this.isStatic = true;
    }
